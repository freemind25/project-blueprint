  ...
  const analyzeText = useCallback((text: string): AIAnalysisResult => {
    ...
    const score = clamp(
      burstinessScore * 0.2 +
        ...
        vocabularyScore * 0.1 +
        depthScore * 0.1 +
        Math.min(40, patternPoints) // bonus de pénalité plafonné pour les motifs explicites
    );

    const humanizationScore = clamp(100 - score);

    // Score SUCKS (Specific, Unique, Clear, Simple, Sticky) — heuristique locale 0-100.
    const specific = clamp(100 - concreteDensity * 0 - depthScore * 0.6 + (digits + properNouns > 3 ? 20 : 0));
