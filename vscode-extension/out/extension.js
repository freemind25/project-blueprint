...
function analyzeComment(text) {
    ...
    for (const p of aiPatterns) {
        ...
        if (matches)
            patternPoints += matches.length * 8;
    }
    ...
    if (hasListMarker)
        perfectionBonus += 5;
    if (startsWithCapital && !hasListMarker)
        perfectionBonus += 2;
    ...
    return Math.min(100, Math.round((patternPoints * 0.4) + (perfectionBonus * 0.2) + (genericDensity * 0.2) + (connCount * 5 * 0.2)));
}
function extractComments(doc) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (/^\s*\/\//.test(trimmed) || (doc.languageId === "python" && /^\s*#(?!\/)/.test(trimmed)) || (doc.languageId === "html" && /<!--/.test(trimmed))) {
            ...
            if (ct.length > 10)
                comments.push({ text: ct, line: i, type: "line", language: doc.languageId });
        }
    }
    return comments;
}
...
function scoreLevel(s) {
    if (s >= 70)
        return { color: "#ef4444", msg: `IA: ${s}% — Forte probabilité` };
    if (s >= 40)
        return { color: "#f59e0b", msg: `IA: ${s}% — Possible` };
    return { color: "#22c55e", msg: `IA: ${s}%` };
}
function makeDecos(comments, threshold) {
...
function activate(ctx) {
    ...
    let diagColl;
    function updateDecos(editor) {
        if (!cfg.get("enableDecorations")) {
            editor.setDecorations(decoType, []);
            return;
        }
        const coms = extractComments(editor.document);
        editor.setDecorations(decoType, makeDecos(coms, cfg.get("threshold")));
    }
    function scanDoc(doc) {
        const coms = extractComments(doc), thr = cfg.get("threshold"), diags = [];
        for (const c of coms) {
            const s = analyzeComment(c.text);
            if (s >= thr) {
                diags.push(new vscode.Diagnostic(new vscode.Range(c.line, 0, c.line, c.text.length), `Commentaire possiblement IA (${s}%)`, s >= 70 ? vscode.DiagnosticSeverity.Warning : vscode.DiagnosticSeverity.Information));
            }
        }
        return diags;
    }
    diagColl = vscode.languages.createDiagnosticCollection("unrobot");
    ctx.subscriptions.push(vscode.commands.registerCommand("unrobot.scanFile", () => {
        ...
        updateDecos(ed);
        const coms = extractComments(ed.document), flagged = coms.filter(c => analyzeComment(c.text) >= cfg.get("threshold"));
        ...
        out.appendLine("[UnRobot] Analyse workspace...");
        out.show();
        ...
        if (vscode.window.activeTextEditor) {
            vscode.window.activeTextEditor.setDecorations(decoType, []);
            diagColl.clear();
        }
    }), vscode.window.onDidChangeActiveTextEditor(ed => { if (ed)
        updateDecos(ed); }), vscode.workspace.onDidSaveTextDocument(d => {
        if (cfg.get("scanOnSave")) {
            diagColl.set(d.uri, scanDoc(d));
            const ed = vscode.window.visibleTextEditors.find(e => e.document.uri.toString() === d.uri.toString());
            if (ed)
                updateDecos(ed);
        }
    }), vscode.workspace.onDidChangeConfiguration(e => { if (e.affectsConfiguration("unrobot") && vscode.window.activeTextEditor)
